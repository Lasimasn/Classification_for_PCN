...
model=pickle.load(open('lrtfidf19.pkl','rb'))
...
def send_mail(email):
    ...
    server.ehlo()
    server.ehlo()
    server.login('nhs.fortest@gmail.com', '')
...
if submitted:
    ...
    labels = ['Pharmacy', 'Physio', 'GP', 'Podiatry', 'Mental Health', 'Social Counselling']
    ans = labels[answer]
    st.write("You are being referred to {dept}. We will get back to you shortly with your appointmnet details.".format(dept=ans))
